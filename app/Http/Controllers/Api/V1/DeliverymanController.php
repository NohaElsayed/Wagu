...
class DeliverymanController extends Controller
{
    ...
    {
        $dm = DeliveryMan::with(['rating'])->where(['auth_token' => $token])->first();
    ...
    public function update_profile(Request $request)
    {
        ...
        $dm->phone1 = $request->phone1;
        $dm->break = $request->break;
    ...
    public function get_current_orders($token)
    {
    ...
    public function get_latest_orders($token)
    {
        $dm = DeliveryMan::where(['auth_token' => $token])->first();
    ...
    public function get_order_history($token,$order_id)
    {
        // $validator = Validator::make($request->all(), [
        //     'order_id' => 'required'
        // ]);
        ...
        $history = DeliveryHistory::where(['order_id' => $order_id, 'delivery_man_id' => $dm['id']])->get();
    ...
    public function get_order_details($token,$order_id)
    {
        // $validator = Validator::make($request->all(), [
        //     'order_id' => 'required'
        // ]);
        ...
        $order = Order::with(['details'])->where(['delivery_man_id' => $dm['id'], 'id' => $order_id])->Notpos()->first();
        if(!$order)
        {
            return response()->json([
                'errors' => [
    ...
    public function get_all_orders($token,$limit,$offset)
    {
        // $validator = Validator::make($request->all(), [
        //     'limit' => 'required',
        //     'offset' => 'required',
