...
Route::group(['namespace' => 'Api\V1', 'middleware'=>'localization'], function () {
    ...
    Route::group(['prefix' => 'auth', 'namespace' => 'Auth'], function () {
        ...
        Route::group(['prefix' => 'vendor'], function () {
            ...
            Route::post('reset-password', 'VendorPasswordResetController@reset_password_submit');
        });
    ...
    Route::group(['prefix' => 'delivery-man'], function () {
        ...
        // Get API's
        Route::get('profile/{token}', 'DeliverymanController@get_profile');
        Route::get('notifications/{token}', 'DeliverymanController@get_notifications');
        Route::get('current-orders/{token}', 'DeliverymanController@get_current_orders');
        Route::get('latest-orders/{token}', 'DeliverymanController@get_latest_orders');
        Route::get('all-orders/{token}/{limit}/{offset}', 'DeliverymanController@get_all_orders');
        Route::get('order-delivery-history/{token}/{order_id}', 'DeliverymanController@get_order_history');
        Route::get('order-details/{token}/{order_id}', 'DeliverymanController@get_order_details');
        ...
        Route::group(['middleware'=>['dm.api']], function () {
            ...
            Route::post('accept-order', 'DeliverymanController@accept_order');
            Route::post('update-order-status', 'DeliverymanController@update_order_status');
            Route::post('update-payment-status', 'DeliverymanController@order_payment_status_update');
            Route::post('update-fcm-token', 'DeliverymanController@update_fcm_token');
        });
    });
    
    Route::group(['prefix' => 'vendor', 'namespace' => 'Vendor', 'middleware'=>['api']], function () {
        ...
        Route::post('request-withdraw', 'VendorController@request_withdraw');
         ...
         // create-order
        Route::post('place-order',[App\Http\Controllers\Api\V1\Vendor\OrderController::class,'place_order']);
