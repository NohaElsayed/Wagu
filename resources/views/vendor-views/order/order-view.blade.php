                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                    ...
                                    <a class="dropdown-item {{$order['order_status']=='confirmed'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'confirmed'])}}','Change status to confirmed ?')"
                                    ...
                                    <a class="dropdown-item {{$order['order_status']=='picked_up'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'picked_up'])}}','Change status to out for delivery ?')"
                                       href="javascript:">{{__('messages.out_for_delivery')}}</a>
                                    <a class="dropdown-item {{$order['order_status']=='delivered'?'active':''}}"
                                       onclick="route_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','Change status to delivered (payment status will be paid if not)?')"
                            ...
                            <div class="col-12" id="items">
                                <div class="d-flex flex-wrap mt-2 mb-3" style="justify-content: space-around;">
                                    @foreach($products as $product)
                                        <div class="item-box">
                                            @include('vendor-views.order.partials._single_product',['product'=>$product, 'restaurant_data'=>$order->restaurant])
                                            {{--<hr class="d-sm-none">--}}
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                @if($editing)
                                    <div class="avatar avatar-xl mr-3 cursor-pointer" onclick="quick_view_cart_item({{$key}})" title="{{__('messages.click_to_edit_this_item')}}">
                                        ...
                                        <img class="img-fluid"
                                             ...
                                             onerror="this.src='{{asset('public/assets/vendor/img/160x160/img2.jpg')}}'"
                        ...
                        ?>
                            <!-- Media -->
                                {{-- <div class="media">
                                    ...
                                    @else
                                    <a class="avatar avatar-xl mr-3" href="{{route('vendor.campaign.view', ['item', $detail->campaign['id']])}}">
                                        <img class="img-fluid"
                                             src="{{asset('storage/app/public/campaign')}}/{{$detail->campaign['image']}}"
                                             onerror="this.src='{{asset('public/assets/vendor/img/160x160/img2.jpg')}}'"
                                             alt="Image Description">
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-2">
                    ...
                    @if($order->delivery_man)    
                        <a class="media align-items-center  deco-none" href="{{route('vendor.delivery-man.preview',[$order->delivery_man['id']])}}">                            
                            <div class="avatar avatar-circle mr-3">
    
                                    <img class="avatar-img" style="width: 75px"
                                    onerror="this.src='{{asset('public/assets/vendor/img/160x160/img1.jpg')}}'"
                ...
                <div class="card mb-2">
                    ...
                    {{-- @if($order->customer)
                        <div class="card-body">
                            ...
                            <a class="media align-items-center deco-none" href="{{route('vendor.customer.view',[$order->customer['id']])}}">    
                                <div class="avatar avatar-circle mr-3">
                ...
                <div class="card">
                    ...
                    {{-- @if($order->restaurant)
                    <div class="card-body">
                        <a class="media align-items-center deco-none" href="{{route('vendor.vendor.view',[$order->restaurant['id']])}}">
                            <div class="avatar avatar-circle mr-3">
                                <img
    ...
    <div id="shipping-address-modal" class="modal fade" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                @if(isset($address))
                    <form action="{{route('vendor.order.update-shipping',[$order['id']])}}"
                          method="post">
                        @csrf
                        <div class="modal-body">
                            <div class="row mb-3">
                                ...
                                @foreach ($deliveryMen as $dm)
                                    <li class="list-group-item"> 
                                        <span class="dm_list" role='button' data-id="{{$dm['id']}}">
                                            <img class="avatar avatar-sm avatar-circle mr-1" onerror="this.src='{{asset('public/assets/vendor/img/160x160/img1.jpg')}}'" src="{{asset('storage/app/public/delivery-man')}}/{{$dm['image']}}" alt="{{$dm['name']}}">
                                            {{$dm['name']}}
                                        </span>    
...
@push('script_2')
    ...
    <script>
        ...
        function removeFromCart(key) {
            Swal.fire({
                ...
                if (result.value) {
                    $.post('{{ route('vendor.order.remove-from-cart') }}', {_token: '{{ csrf_token() }}', key: key, order_id: '{{$order->id}}'}, function (data) {
                        if (data.errors) {
    ...
    <script>
        function addDeliveryMan(id) {
            $.ajax({
                type: "GET",
                url: '{{url('/')}}/vendor/order/add-delivery-man/{{$order['id']}}/' + id,
                success: function (data) {
                    location.reload();
                    console.log(data)
    ...
    <script>
        ...
        function initializeGMap() {
            ...
            var Restaurantmarker = new google.maps.Marker({
                ...
                icon: "{{asset('public/assets/vendor/img/restaurant_map.png')}}"
            });
            ...
            for (var i = 0; i < deliveryMan.length; i++) {                  
                if(deliveryMan[i].lat)
                {
                    ...
                    var marker = new google.maps.Marker({
                        ...
                        icon: "{{asset('public/assets/vendor/img/delivery_boy_map.png')}}"
                    });
        ...
        $(document).ready(function() {
            ...
            function initializegLocationMap() {
                ...
                var marker = new google.maps.Marker({
                    ...
                    icon: "{{asset('public/assets/vendor/img/customer_location.png')}}"
                });
