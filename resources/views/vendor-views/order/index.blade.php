					...
					<div class="card-body" id="items">
                        <div class="d-flex flex-wrap mt-2 mb-3" style="justify-content: space-around;">
                            @foreach($products as $product)
                                <div class="item-box">
                                    @include('vendor-views.order._single_product',['product'=>$product, 'restaurant_data'=>$restaurant_data])
                                    {{--<hr class="d-sm-none">--}}
                                </div>
                            @endforeach
                        </div>
				...
				<div class="col-md-4">
                    <div class="card">
                        ...
                        <div class='w-100' id="cart">
                            @include('vendor-views.order._cart')
                        </div>
                    </div> 
				</div>
			</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body row" style="font-family: emoji;">
                    ...
                    <div class="row" id="printableArea" style="margin: auto;">
                        @include('vendor-views.order.invoice')
                    </div>
...
<script>
    ...
    function quickView(product_id) {
        $.get({
            url: '{{route('vendor.addorder.quick-view')}}',
            dataType: 'json',
            data: {
    ...
    function quickViewCartItem(product_id, item_key) {
        $.get({
            url: '{{route('vendor.addorder.quick-view-cart-item')}}',
            dataType: 'json',
            data: {
                product_id: product_id,
                item_key: item_key
    ...
    function getVariantPrice() {
        if ($('#add-to-cart-form input[name=quantity]').val() > 0 && checkAddToCartValidity()) {
            ...
            $.ajax({
                type: "POST",
                url: '{{ route('vendor.addorder.variant_price') }}',
    ...
    function addToCart(form_id = 'add-to-cart-form') {
        if (checkAddToCartValidity()) {
            ...
            $.post({
                url: '{{ route('vendor.addorder.add-to-cart') }}',
    ...
    function removeFromCart(key) {
        $.post('{{ route('vendor.addorder.remove-from-cart') }}', {_token: '{{ csrf_token() }}', key: key}, function (data) {
            if (data.errors) {
    ...
    function emptyCart() {
        $.post('{{ route('vendor.addorder.emptyCart') }}', {_token: '{{ csrf_token() }}'}, function (data) {
            updateCart();
    ...
    function updateCart() {
        $.post('<?php echo e(route('vendor.addorder.cart_items')); ?>', {_token: '<?php echo e(csrf_token()); ?>'}, function (data) {
   ...
   $(function(){
    ...
    function updateQuantity(e){
        ...
        if (valueCurrent >= minValue) {
            $.post('{{ route('vendor.addorder.updateQuantity') }}', {_token: '{{ csrf_token() }}', key: key, quantity:valueCurrent}, function (data) {
                updateCart();
            });
        } else {
            Swal.fire({
    ...
    $('.js-data-example-ajax').select2({
        ajax: {
            url: '{{route('vendor.addorder.customers')}}',
